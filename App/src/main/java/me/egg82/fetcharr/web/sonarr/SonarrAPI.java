...
public class SonarrAPI extends AbstractArrAPI {
    ...
    public void search(int... itemIds) {
        for (int itemId : itemIds) {
            ...
            if (response == null) {
                logger.warn("Sonarr returned invalid response for URL {}: null", baseUrl + "/api/" + version() + "/command");
                return;
            }
            int id = NumberParser.parseInt(-1, StringParser.parse(response.getObject(), "id"));
            if (id < 0) {
                logger.warn("Sonarr returned unexpected response for URL {}: {}", baseUrl + "/api/" + version() + "/command", response.getObject().toString());
            }
        }
    }
}
