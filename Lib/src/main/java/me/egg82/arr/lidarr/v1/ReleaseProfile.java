...
public class ReleaseProfile extends AbstractFetchableAPIObject {
    ...
    private final PVector<@NotNull ReleaseProfileResource> resources;
    ...
    public ReleaseProfile(@NotNull ArrAPI api, @NotNull JsonNode node, @NotNull Instant lastFetched) {
        ...
        if (node.isArray()) {
            ...
            List<@NotNull ReleaseProfileResource> resourcesL = new ArrayList<>();
            for (int i = 0; i < resources.length(); i++) {
                resourcesL.add(new ReleaseProfileResource(api, resources.getJSONObject(i)));
            }
            this.resources = TreePVector.from(resourcesL);
        } else {
            this.resources = TreePVector.singleton(new ReleaseProfileResource(api, node.getObject()));
        }
    }
    ...
    public @NotNull PVector<@NotNull ReleaseProfileResource> resources() {
        return resources;
    }
