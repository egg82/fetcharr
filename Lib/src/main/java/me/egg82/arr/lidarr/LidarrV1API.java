...
public class LidarrV1API extends AbstractArrAPI {
    ...
    public boolean valid() {
        ...
        if (current == null || !current.equalsIgnoreCase(version())) {
            logger.warn("LIDARR_{} returned unexpected response for URL {}: {}", this.id, this.baseUrl + "/api", response.getObject().toString());
            return false;
        }
        return true;
    }
    ...
    public void search(int... itemIds) {
        for (int itemId : itemIds) {
            ...
            if (id < 0) {
                logger.warn("LIDARR_{} returned unexpected response for URL {}: {}", this.id, this.baseUrl + "/api/" + version() + "/command", response.getObject().toString());
            }
        }
    }
