...
public class Track extends AbstractFetchableAPIObject {
    ...
    private final PVector<@NotNull TrackResource> resources;
    ...
    public Track(@NotNull ArrAPI api, @NotNull JsonNode node, @NotNull Instant lastFetched) {
        ...
        if (node.isArray()) {
            ...
            List<@NotNull TrackResource> resourcesL = new ArrayList<>();
            for (int i = 0; i < resources.length(); i++) {
                resourcesL.add(new TrackResource(api, resources.getJSONObject(i)));
            }
            this.resources = TreePVector.from(resourcesL);
        } else {
            this.resources = TreePVector.singleton(new TrackResource(api, node.getObject()));
        }
    }
    ...
    public @NotNull PVector<@NotNull TrackResource> resources() {
        return resources;
    }
