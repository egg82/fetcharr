...
public class Artist extends AbstractFetchableAPIObject {
    ...
    private final List<@NotNull ArtistResource> resources = new ArrayList<>();
    ...
    public Artist(@NotNull ArrAPI api, @NotNull JsonNode node, @NotNull Instant lastFetched) {
        ...
        if (node.isArray()) {
            ...
            for (int i = 0; i < resources.length(); i++) {
                this.resources.add(new ArtistResource(api, resources.getJSONObject(i)));
            }
        } else {
            this.resources.add(new ArtistResource(api, node.getObject()));
        }
    }
