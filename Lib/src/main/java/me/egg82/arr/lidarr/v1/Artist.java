...
public class Artist extends AbstractFetchableAPIObject {
    public static final Artist UNKNOWN = new Artist();

    private final PVector<@NotNull ArtistResource> resources;

    public Artist(@NotNull ArrAPI api, @NotNull JsonNode node, @NotNull Instant lastFetched) {
        super(api, node, lastFetched);

        if (node.isArray()) {
            JSONArray resources = node.getArray();
            List<@NotNull ArtistResource> resourcesL = new ArrayList<>();
            for (int i = 0; i < resources.length(); i++) {
                resourcesL.add(new ArtistResource(api, resources.getJSONObject(i)));
            }
            this.resources = TreePVector.from(resourcesL);
        } else {
            this.resources = TreePVector.singleton(new ArtistResource(api, node.getObject()));
        }
    }
    ...
    public @Nullable ArtistResource resource() {
